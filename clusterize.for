...
c     &'Alkylating Agents   ','Cyanomorpholinodoxorubicin    ', 357704,  
     ...
     &'DNA Antimetabolites ','thiopurine                    ',    755/  
      ...
      do i=1,n_ref
       write(*,*) i,agent_name(i),agent_number(i),action_mode(i)
      enddo
     ...
     &    covariation+activity_cen(seed1,k)*activity_cen(seed2,k)
	  ...
	  correlation(i,j)=-100.0
        n_compare(i,j)=0
        write(*,*) 'warning! insufficient data for correlation!'
       else
...
c      enddo
	 ...
	 do j=1,n_ref
      ...
      write(*,*) "minimal correlation: ", min_corr
...
c      enddo
	 ...
	 if(cluster_n_old.NE.cluster_n(i)) convergence=.FALSE.
      ...
      write(*,*) iter,convergence
      enddo      


      do i=1,n_ref
      write(*,*) i, cluster_n(i), action_mode(i),agent_name(i)
      enddo
...
c      enddo
      ...
      do i=1,n_ref
      write(*,fmt='(F10.6,2X,F10.6,2X,F10.6,2X,F10.6,2X,F10.6
     &	,2X,A20,2X,A30)')
     &	 real(evec(i,1)),real(evec(i,2)),real(evec(i,3)),
     &     real(evec(i,4)), real(evec(i,5)),
     &	action_mode(i),agent_name(i)
      evec1_2=evec1_2 + evec(i,1)**2
      evec2_2=evec2_2 + evec(i,2)**2
      evec3_2=evec3_2 + evec(i,3)**2
      evec12=evec12 + evec(i,1)*evec(i,2)
      evec13=evec13 + evec(i,1)*evec(i,3)
      evec23=evec23 + evec(i,2)*evec(i,3)

      enddo
      write(*,*)'mod,1,2,3:',sqrt(evec1_2),sqrt(evec2_2),sqrt(evec3_2)
      write(*,*)'mod,12,13,23:',evec12,evec13,evec23
...
c      correlation_m  =   correlation(max_ref)
c	agent_number_m =  agent_number(max_ref)
c      action_mode_m  =   action_mode(max_ref)
c	agent_name_m   =    agent_name(max_ref)
c      n_compare_m    =    n_compare(max_ref)
